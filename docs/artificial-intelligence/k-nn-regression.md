...
· 그림에는 분류되지 않은 데이터(빨간 점)과 Class A로 분류가 된 데이터(노란 점)와 Class B로 분류가 된 데이터(보라 점)가 있습니다. 이제 분류가 되지 않은 데이터 (빨간 점)를 어떤 Class로 분류할 지 생각해보아야 합니다. K-최근접 이웃(K-Nearest Neighbor) 알고리즘은 주변에 있는 점을 하나 씩 거리를 측정해가며 가장 가까운 'k'개의 데이터(즉, 이웃)를 찾습니다. 거리는 주로 유클리드 거리로 측정되지만 다른 거리 측정 방법도 사용될 수 있습니다. 
